...
export interface NAIMetadata {
    ...
    // Source info
    source?: 'text_chunk' | 'stealth_alpha' | 'webp_chunk'
...
export async function parseNAIMetadata(imageData: ArrayBuffer | Uint8Array): Promise<NAIMetadata | null> {
    try {
        ...
        const riffSignature = String.fromCharCode(...bytes.slice(0, 4))
        const webpSignature = String.fromCharCode(...bytes.slice(8, 12))
        if (riffSignature === 'RIFF' && webpSignature === 'WEBP') {
            const { extractWebPMetadata } = await import('./metadata-writer-webp')
...
async function extractTextChunkMetadata(bytes: Uint8Array): Promise<NAIMetadata | null> {
    ...
    // Add model source if found, mapping display name to API ID
    if (metadata && modelSource) {
        metadata.model = mapModelDisplayNameToId(modelSource)
    }
...
/**
 * Map NovelAI's display model names to API Model IDs
 * The PNG "Source" chunk contains human-readable names like "NovelAI Diffusion V4.5 5BB76870"
 * but the API expects IDs like "nai-diffusion-4-5-full"
 */
function mapModelDisplayNameToId(displayName: string): string {
    const lowerName = displayName.toLowerCase()

    // V4.5 models
    if (lowerName.includes('v4.5') || lowerName.includes('4.5')) {
        if (lowerName.includes('curated')) {
    ...
    // V4 models
    if (lowerName.includes('v4') || lowerName.includes('diffusion 4') || lowerName.includes('diffusion-4')) {
        if (lowerName.includes('curated')) {
    ...
    // V3 Furry
    if (lowerName.includes('furry')) {
        return 'nai-diffusion-furry-3'
    }

    // V3 (default)
    if (lowerName.includes('v3') || lowerName.includes('diffusion 3') || lowerName.includes('diffusion-3')) {
        return 'nai-diffusion-3'
    }

    // If it already looks like an API ID, return as-is
    if (displayName.startsWith('nai-diffusion')) {
        return displayName
    }

    // Fallback: return as-is (could be a newer model not yet mapped)
    console.warn(`Unknown model display name: ${displayName}, using as-is`)
    return displayName
}
